...
NEXUS_TAR="${NEXUS_NAME}-linux-x86_64.tar.gz"
...
fail_if_error sudo apt update -y
fail_if_error sudo apt upgrade -y
...
if command -v java &> /dev/null; then
    ...
    fail_if_error sudo apt install -y "$JAVA_PACKAGE"
fi
...
if ! getent group "$NEXUS_GROUP" > /dev/null; then
    ...
    echo "ℹ️ Group $NEXUS_GROUP already exists"
fi
...
if ! id "$NEXUS_USER" > /dev/null 2>&1; then
    ...
    echo "ℹ️ User $NEXUS_USER already exists"
fi
...
fail_if_error sudo systemctl daemon-reload
